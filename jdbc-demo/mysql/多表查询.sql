...
-- select emp.ename, emp.salary, t2.grade from emp, salarygrade t2 where emp.salary between t2.losalary and t2.hisalary;
...
/*
	分析:
		...
		4. 部门名称, 部门位置 来自于部门表 dept表中
		5. dept部门表和emp员工表 是一对多的关系, dept.id = emp.dept_id.
		6. 工资等级信息在salarygrade工资等级表中
		7. emp.salary >= salarygrade.losalary and emp.salary <= salarygrade.hisalary
*/
-- select emp.ename, emp.salary, job.jname, job.description , dept.dname, dept.loc, t2.grade from emp inner join job on emp.job_id = job.id inner join dept on dept.id = emp.dept_id inner join salarygrade t2 on emp.salary between t2.losalary and t2.hisalary;

-- 5. 查询出部门编号, 部门名称, 部门位置, 部门人数
/*
	分析:
		1. 部门编号, 部门名称, 部门位置信息来自于dept部门表中.
		2. 部门人数: 在emp表中按照dept_id进行分组, 然后count(*)统计数量. 
		3. 使用子查询, 让部门表和分组后的部门人数表进行内连接.
*/
